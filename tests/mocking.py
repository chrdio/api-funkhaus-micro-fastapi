...
b_special_cases_or_alternatives = st.lists(st.booleans(), min_size=LENGTH, max_size=LENGTH)
...
b_cheetsheet = st.builds(
	...
	info=st.lists(
		st.tuples(
			st.sampled_from(tuple(NodeIDs)), st.sampled_from(tuple(ChordSymbolStructures))
		),
		min_size=LENGTH,
		max_size=LENGTH,
	),
