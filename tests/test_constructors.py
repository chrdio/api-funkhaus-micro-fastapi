...
@given(
    progression=b_progression_fields, cheetsheet=b_cheetsheet, pseudomidi=b_pseudomidi
)
...
@given(
    st.builds(API.GenericRequest, user_object=b_user_obj, sess_id=st.ip_addresses(v=4))
)
...
@given(
    st.builds(API.GenericRequest, user_object=st.none(), sess_id=st.ip_addresses(v=4))
)
...
def test_construct_performance(progression, chsh, psdm, hex_blob):
    assert API.construct_performance(
        progression=progression, cheet_sheet=chsh, pseudo_midi=psdm, hex_blob=hex_blob
    )
